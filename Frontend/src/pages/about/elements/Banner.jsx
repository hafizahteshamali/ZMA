...
const AnimatedLetters = ({ text, className = "" }) => {
  ...
  const letter = {
    ...
    visible: {
      ...
      transition: { 
        duration: 0.5, 
        ease: [0.16, 0.77, 0.47, 0.97] // Custom easing for smoother bounce
      },
    },
  };
        ...
        >
          <div className="container mx-auto w-full px-4 sm:px-6 lg:px-8 h-full py-8 md:py-12 lg:py-16">
            ...
            >
              ...
              {/* Right Column */}
              <motion.div 
                className="w-full lg:w-[60%] flex flex-col gap-4 sm:gap-6"
                variants={containerVariants}
              >
                <motion.p 
                  className="text-sm sm:text-base md:text-lg text-[var(--text-hover-color)] font-normal"
                  variants={itemVariants}
                >
